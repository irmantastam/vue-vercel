    ...
    >
      <div className="flex-1 basis-1/2" {...inspectorProps({ fieldId: 'featuredImage' })}>
        {article.featuredImage && (
          <CtfImage
            nextImageProps={{ className: 'w-full', priority: true }}
            {...article.featuredImage}
          />
        )}
      </div>
      ...
      <div className="relative flex flex-1 basis-1/2 flex-col justify-center py-6 px-4 lg:px-16 lg:py-12 xl:px-24">
        <div className="mb-2 flex flex-wrap items-center">
          ...
          <div
            className={twMerge('ml-auto hidden pl-2 text-xs', isReversedLayout ? 'lg:block' : '')}
        ...
        <div
          className={twMerge('mt-2 text-xs', isReversedLayout ? 'lg:hidden' : '')}
          {...inspectorProps({ fieldId: 'publishedDate' })}
        >
          <FormatDate date={publishedDate} />
        </div>
